...
from domain.browser import BrowserState, SensitiveBrowserOperation
...
class ReliablePlaywrightBrowserProvider(PlaywrightBrowserProvider):
    """Production Playwright adapter with resilient location and post-action evidence.

    Potentially state-changing actions are never blindly replayed. Only actions explicitly
    allowed by BrowserReliabilityPolicy receive one transient retry.
    """
    ...
    async def _wait_dom(page: Any) -> None:
        try:
            ...
            # Dynamic applications may never become globally idle. Locator readiness below is authoritative.
            pass
    ...
    async def _resolve_locator(self, page: Any, selector: str):
        ...
        # Prefer the concrete selector emitted by the browser snapshot. If DOM churn made it
        # stale or invalid, recover semantically from the supplied human-readable value.
        try:
    ...
    async def click(self, session_id: str, selector: str) -> BrowserState:
        # Never retry click: a timeout after dispatch can be ambiguous and replaying may duplicate side effects.
        session, page = await self._active(session_id)
